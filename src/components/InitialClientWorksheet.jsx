...
import {
  ...
  setExporting,
  resetForm
} from '../redux/slices/initialClientWorksheetSlice';

import invoiceTemplate from '../../templates/initialClientWorksheet.html';

const InitialClientWorksheet = () => {
  const dispatch = useDispatch();
  const { formData, isExporting } = useSelector((state) => state.initialClientWorksheet);

  const handleChange = (section, field, value) => {
    dispatch(updateFormData({ section, field, value }));
  };

  const handleNestedChange = (section, subsection, field, value) => {
    dispatch(updateNestedFormData({ section, subsection, field, value }));
  };

  const handleComponentChange = (section, subsection, component, value) => {
    dispatch(updateComponentData({ section, subsection, component, value }));
  };

  const handleAddCustomOption = (section, subsection, option) => {
    if (option && option.trim() !== '') {
      dispatch(addCustomOption({ section, subsection, option }));
    }
  };

  const handleRemoveCustomOption = (section, subsection, optionId) => {
    dispatch(removeCustomOption({ section, subsection, optionId }));
  };

  const handleToggleCustomOption = (section, subsection, optionId) => {
    dispatch(toggleCustomOption({ section, subsection, optionId }));
  };

  const handleAddOtherRequirement = (requirement) => {
    if (requirement && requirement.trim() !== '') {
      dispatch(addOtherRequirement({ requirement }));
    }
  };

  const handleRemoveOtherRequirement = (index) => {
    dispatch(removeOtherRequirement({ index }));
  };

  const handleExportPDF = async () => {
    setLoading(true);
    try {
      // Send the imported HTML content to your API
      const response = await axios.post('http://localhost:5000/pdf/convert-html-to-pdf', {
        htmlFile: invoiceTemplate,
        htmlFile: 'invoice.pdf'
      }, {
        responseType: 'blob'
      });
  ...
  //     formDataObj.append('htmlFile', dataBlob, '../templates/initialClientWorksheet.html');
  //     // formDataObj.append('url', 'http://localhost:5173/templates/initialClientWorksheet.html');

  //     const response = await fetch('http://localhost:5000/pdf/convert-html-to-pdf', {
  //       method: 'POST',
  //       body: formDataObj 
  //     });
  ...
  //     const link = document.createElement('a');
  //     link.href = blobUrl;
  //     link.download = 'securethread-website.pdf';
  ...
  const handlePreview = () => {
    // Create a new window/tab with the preview
    const previewWindow = window.open('/templates/initialClientWorksheet.html', '_blank');
    
    // Wait for the window to load before updating the form data
    previewWindow.onload = () => {
      // Call the updateFormData function defined in the HTML template
      previewWindow.updateFormData(formData);
    };
  };
            ...
            {formData.clientRequirements.frap.customOptions.map(option => (
              <div key={option.id} className="flex items-center">
                <input
                  ...
                  onChange={() => handleToggleCustomOption('clientRequirements', 'frap', option.id)}
                />
                <label htmlFor={`frap-${option.id}`} className="flex-grow">{option.label}</label>
                <button
                  type="button"
                  className="text-red-500 ml-2"
                  onClick={() => handleRemoveCustomOption('clientRequirements', 'frap', option.id)}
                >
                  ✕
                </button>
              </div>
          ...
          <div className="mt-4 flex">
            ...
            <button
              ...
              onClick={() => {
                ...
                handleAddCustomOption('clientRequirements', 'frap', input.value);
                input.value = '';
              }}
            >
              Add
            ...
            {formData.clientRequirements.externalVulnerabilityAssessment.customOptions.map(option => (
              <div key={option.id} className="flex items-center mt-2">
                <input
                  ...
                  className="mr-2"
                  checked={option.checked}
                  onChange={() => handleToggleCustomOption('clientRequirements', 'externalVulnerabilityAssessment', option.id)}
                />
                <label htmlFor={`eva-${option.id}`} className="flex-grow">{option.label}</label>
                <button
                  type="button"
                  className="text-red-500 ml-2"
                  onClick={() => handleRemoveCustomOption('clientRequirements', 'externalVulnerabilityAssessment', option.id)}
                >
                  ✕
                </button>
              </div>
            ...
            <div className="mt-4 flex">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  handleAddCustomOption('clientRequirements', 'externalVulnerabilityAssessment', input.value);
                  input.value = '';
                }}
              >
                Add
            ...
            {formData.clientRequirements.gapAnalysis.customOptions.map(option => (
              <div key={option.id} className="flex items-center">
                <input
                  ...
                  className="mr-2"
                  checked={option.checked}
                  onChange={() => handleToggleCustomOption('clientRequirements', 'gapAnalysis', option.id)}
                />
                <label htmlFor={`gap-${option.id}`} className="flex-grow">{option.label}</label>
                <button
                  type="button"
                  className="text-red-500 ml-2"
                  onClick={() => handleRemoveCustomOption('clientRequirements', 'gapAnalysis', option.id)}
                >
                  ✕
                </button>
              </div>
          ...
          <div className="mt-4 flex">
            ...
            <button
              ...
              onClick={() => {
                ...
                handleAddCustomOption('clientRequirements', 'gapAnalysis', input.value);
                input.value = '';
              }}
            >
              Add
            ...
            <div className="flex">
              ...
              <button
                ...
                onClick={(e) => {
                  ...
                  handleAddOtherRequirement(input.value);
                  input.value = '';
                }}
              >
                Add
